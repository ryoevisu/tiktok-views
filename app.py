...
[bold white]Views :[bold green] +{self.views_sent}""", width=56, style="bold bright_white", title="[bold bright_white][ Success ]"))
    ...
    def DECRYPT_BASE64(self, base64_code):
...
class MAIN:
    ...
    def __init__(self):
        try:
            self.DISPLAY_LOGO()
            printf(Panel(f"[bold white]Please fill in your TikTok video link, make sure the account is not private and the\nlink is correct. Take the video link via browser!", width=56, style="bold bright_white", title="[bold bright_white][ Video Link ]", subtitle="[bold bright_white]╭─────", subtitle_align="left"))
